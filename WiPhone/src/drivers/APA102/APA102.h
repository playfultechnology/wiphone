...
#define APA102_DELAY delayNops(35)

static void IRAM_ATTR delayNops(uint32_t nops)
{
    while (nops--) {
        NOP();
    }
...
namespace Pololu
{
  ...
  protected:
    ...
    void transfer(uint8_t b)
    {
      ...
      /*
      if (b >> 7 & 1)
          GPIO.out_w1ts = ((uint32_t)1 << dataPin);
      else
          GPIO.out_w1tc = ((uint32_t)1 << dataPin);
      RGB_DELAY;
      GPIO.out_w1ts = ((uint32_t)1 << clockPin);
      RGB_DELAY;
      GPIO.out_w1tc = ((uint32_t)1 << clockPin);
      RGB_DELAY;
